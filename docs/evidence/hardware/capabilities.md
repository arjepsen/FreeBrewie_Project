...
| Modern experimental WLAN | A Realtek `0bda:8176` USB device enumerated, loaded `rtl8192cu`, and associated during the dated runtime capture. [HW-107](facts/HW-107.md) | Does not independently verify physical fitment; not a permanent hardware, throughput, placement, adapter, or driver requirement. |
...
| Networking | A dated modern boot enumerated and used a Realtek `0bda:8176` USB WLAN device with `rtl8192cu`, but physical fitment, permanent networking identity, placement, interface limits, throughput, and future suitability are unresolved. [HW-107](facts/HW-107.md) | verified runtime observation; physical/permanent capability unresolved |
...
Historical partition sizes, EXT3, kernel versions, Buildroot, the old captured watchdog behavior, and the runtime-enumerated Realtek device remain observations rather than selected clean-slate requirements. These limits constrain future evidence gathering only. They do not select an operating system, software architecture, firmware implementation, protocol, brewing behavior, or control policy.
